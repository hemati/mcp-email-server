...
- metamcp deployt via `uvx --from git+https://github.com/hemati/mcp-email-server@vX.Y.Z-scher mcp-email-server-scher stdio`. Nach neuem Tag: in metamcp das `@…`-Argument bumpen + reconnecten. **metamcp hält Server-Konfig und vorgestartete idle sessions im Speicher:** das Argument direkt in Postgres umzustellen (`UPDATE mcp_servers SET args = array_replace(…) WHERE name='scher-mail'`, Host langflux.space, Container `metamcp-postgres`) greift erst nach `docker restart metamcp` (kurz auch Playwright/GA4/Google-Ads weg) — 2026-09-19 bei v0.1.9 gemessen: nach dem DB-Update starteten neue Prozesse weiter mit dem alten Tag. Prüfen: `docker exec metamcp ps -eo args | grep mcp-email-server@`. **`docker logs metamcp` enthält die Server-Env samt Passwort** — nur gefiltert ausgeben. Neuer/geänderter Tool → der claude.ai-Connector cached das Manifest, ggf. Connector-Re-List nötig (nicht nur `/mcp reconnect`).
